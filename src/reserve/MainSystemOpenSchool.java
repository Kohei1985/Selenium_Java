...
public class MainSystemOpenSchool {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        try {
            ...
            for (int i = 30; i <= 30; i++) { //<----エクセルの範囲指定はここ！！1~26までの数字
                ...
                Select dropdown01 = new Select(
                        driver.findElement(By.name("ctl00$ContentPlaceHolder1$ShinseiKumiawaseInp1$drpPurposeBunrui")));
                ...
                for (String reserveDay : reserveDays) {
                    WebElement element10 = driver
                            .findElement(By.xpath("//a[contains(@title,'" + reserveMonth + "月" + reserveDay + "日')]"));
                    ...
                    for (int j = 0; j < 1; j++) {//予約を"j"回繰り返す <---ここの数字を変更で繰り返し回数指定
                        ...
                        int rmInt = Integer.parseInt(reserveMonth); //月をint型に変換※intにしないとゼロ埋めができないため
                        int rdInt = Integer.parseInt(reserveDay); //日をint型に変換
                        String rmStr = String.format("%02d", rmInt); //月をゼロ埋めしてStringに戻す
                        String rdStr = String.format("%02d", rdInt); //日をゼロ埋めしてStringに戻す
                        String youbi = ReserveDateController.getYoubi("2021" + rmStr + rdStr); //曜日を取得する
